...
# ####### NOTE NOTE NOTE NOTE
# in order to solve the "multiple definition of `__floatsisf'" problem
# we need to link the math library AFTER our object code!!!... so
# we MOVED the -lm switch @ $(TRG) target line
# linker
...
writeflash: hex
	$(AVRDUDE) -C $(AVRDUDE_CONFIG) -c $(AVRDUDE_PROGRAMMERID) \
	 -p $(PROGRAMMER_MCU) -P $(AVRDUDE_PORT) -b $(AVRDUDE_SPEED) -e \
	 -U flash:w:$(HEXROMTRG):i -v
